...
export async function updateSingleArtPrintAction(id: string, prevState: any, formData: FormData) {
  ...
  const frameOptions: ArtPrintFrameOption[] = [
    {
      id: 'plain',
  ...
  const galleryImages: ArtPrintGalleryImage[] = [];
  for (let imgIndex = 0; imgIndex < 3; imgIndex++) {
    const url = rawData[`galleryImages[${imgIndex}].url`] as string;
    if (url) {
      galleryImages.push({
        id: `gallery-${id}-${imgIndex}`,
        url: url,
        alt: `Galleribillede ${imgIndex + 1} for ${rawData.name}`,
      });
    }
  }

  const updatedPrintData: ArtPrint = {
    ...allPrints[printIndex], // Keep existing data
    id: id,
  ...
  // 4. Create the new list of all prints
  const newPrintsList = [...allPrints];
  newPrintsList[printIndex] = updatedPrintData;

  // 5. Save the entire new list
  const { success, error } = await updateArtPrints(newPrintsList);
