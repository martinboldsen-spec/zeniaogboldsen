...
export default async function HomePage() {
  ...
  const boldsenArtworks = artworks
    ?.filter(art => art.artist === 'boldsen')
    .sort((a, b) => new Date(b.creationDate).getTime() - new Date(a.creationDate).getTime());
    
  const zeniaArtworks = artworks
  ...
  const {
      ...
      home_intro_content: "Jeg elsker at skabe, og når jeg maler oplever jeg den mest umiddelbare og befriende forskellighed fra min ellers strukturerede hverdag. Farverne er altafgørende ..., der er ingen regler for deres sammensætning ..., hvis det føles rigtigt, så er det rigtigt.\n\nMine malerier giver et godt billede af hvem jeg er. På den ene side et behov for ro og stabilitet ..., men samtidig en konstant tiltrækning til den impulsive og farvestrålende vildskab.",
          ...
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center text-center md:justify-start md:text-left p-4 md:p-12">
            <div className="max-w-md bg-black/50 backdrop-blur-sm p-6 md:p-8 rounded-sm space-y-3">
                 <Image
                    ...
                    priority
                />
                <h1 className="text-3xl md:text-5xl font-bold font-headline text-white drop-shadow-lg">{home_hero_title}</h1>
      ...
      <main className="container mx-auto max-w-7xl px-4 py-8 md:py-12">
        ...
        {latestArtworks && latestArtworks.length > 0 && (
          <section className="mb-8 md:mb-12">
            <h2 className="font-headline text-3xl text-center text-primary mb-8">Seneste Værker</h2>
        ...
        {promo_carousel_active && promo_carousel_slides && promo_carousel_slides.length > 0 && (
          <section className="mb-8 md:mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start">
                ...
                <div>
                    <div className="prose prose-lg max-w-none text-foreground/90">
                        <h2 className="font-headline text-4xl text-primary">{home_intro_title}</h2>
                        {home_intro_content.split('\n').map((paragraph, index) => (
                            <p key={index}>
                                {paragraph}
                            </p>
                        ))}
                        <p className="text-right italic text-muted-foreground mt-6">{home_intro_signature}</p>
                    </div>
                </div>
                    ...
                    <div className="relative aspect-square w-full overflow-hidden">
                     ...
                     <div className="mt-8">
                        <p className="text-foreground/80 mb-6">
                            Galleriet her på siden viser et kurateret udvalg af vores værker. For at se hele kollektionen, specifikke detaljer og købsmuligheder, opfordrer vi dig til at besøge vores individuelle hjemmesider.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="bg-background p-6 border shadow-sm text-center">
                                <h3 className="font-headline text-2xl font-bold text-primary">Martin Boldsen</h3>
                                <p className="text-muted-foreground mt-1">Billedkunstner</p>
                                <Button asChild className="mt-4 w-full">
                                    <Link href={boldsen_website_url || '#'} target="_blank" rel="noopener noreferrer">Besøg Hjemmeside</Link>
                                </Button>
                            </div>
                            <div className="bg-background p-6 border shadow-sm text-center">
                                <h3 className="font-headline text-2xl font-bold text-primary">Anja Zenia</h3>
                                <p className="text-muted-foreground mt-1">Keramiker & Billedkunstner</p>
                                <Button asChild className="mt-4 w-full">
                                    <Link href={zenia_website_url || '#'} target="_blank" rel="noopener noreferrer">Besøg Hjemmeside</Link>
                                </Button>
                            </div>
                        </div>
                    </div>
